...
const AvailableGames = () => {
  ...
  const handleJoin = (gameId) => {
    // Update the game's request status to pending
    MakeRequestAPI(gameId).then((res)=>{
      if(res && res.status === 201){
        navigate(`/request-pending/${gameId}`);
      }else{
           ...
           games.map((game) => (
            <div key={game.id} className="bg-white rounded-lg shadow-md p-6">
              ...
              game.requestStatus === 'approved' ? (
                ...
                onClick={() => navigate(`/select-seat/${game.id}`)}
                >
                  <button className="text-green-600 
                  
                    font-semibold cursor-pointer hover:text-yellow-700 transition duration-200
                    rounded-lg bg-green-100 p-2
                  ">
